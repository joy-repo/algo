...
public class RegularExpressionMatching {
    ...
    public static char[] PATTERN = "xa*byi".toCharArray();
        ...
        for (int c = 1; c <= STR.length; c++) {
            for (int r = 1; r <= PATTERN.length; r++) {
                ...
                if (PATTERN[r - 1] == '.' && DP[c - 1][r - 1])
                    DP[c][r] = true;
                if (PATTERN[r - 1] == '*') {
                    if (STR[c - 1] == PATTERN[r - 2] || PATTERN[r - 2] == '.')
                        DP[c][r] = DP[c - 1][r];
                    DP[c][r] = DP[c][r] || DP[c - 2][r];
                }

//                DP[c][r] = ((STR[c - 1] == PATTERN[r - 2] || PATTERN[r - 2] == '.') && DP[c - 1][r - 1]) || DP[c][r - 2])
//                ;
